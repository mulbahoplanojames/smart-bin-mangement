...
export function AnalyticsOverview({
  ...
  const totalWaste = completedCols.reduce(
    (sum, c) => sum + c.wasteCollected,
    0,
  );
  ...
  return (
    <div className="bg-[#11131a] border border-[#1e2029]/60 p-8 rounded-[2rem] h-fit flex flex-col group hover:bg-[#1a1c25] hover:border-[#3b82f6]/30 transition-all duration-500">
      <div className="flex justify-between items-start mb-10">
        <div>
          <h3 className="text-white font-[950] tracking-tighter leading-none mb-3 text-2xl">
            Analytics Overview
          </h3>
          <div className="flex gap-8 mt-10 flex-wrap">
            <div>
              <p className="text-gray-700 text-[10px] font-black uppercase tracking-[0.2em] mb-2">
                Total Collected
              </p>
              <p className="text-white font-[900] tracking-tighter text-2xl">
                {totalWaste.toFixed(1)}{" "}
                <span className="text-gray-600 font-bold text-sm tracking-normal">
                  ton
                </span>
              </p>
              <p className="text-[#10b981] text-xs font-black mt-2 tracking-tight">
            ...
            <div>
              ...
              <p
                className={`text-xs font-black mt-2 tracking-tight ${efficiency >= 80 ? "text-[#10b981]" : efficiency >= 50 ? "text-[#f59e0b]" : "text-[#ef4444]"}`}
              >
                {efficiency >= 80
                  ? "Excellent"
                  : efficiency >= 50
        ...
        <div className="bg-[#1e2029] px-4 py-2 rounded-xl border border-gray-800 cursor-pointer hover:bg-[#2a2c35] transition-colors flex-shrink-0 flex items-center gap-3">
          <span className="text-gray-400 text-[11px] font-[900] uppercase tracking-[0.1em]">
            This Month
          </span>
