...
import { 
  FiBell, FiRefreshCw, FiAlertCircle, FiAlertTriangle, 
  FiInfo, FiCheckCircle, FiTrash2, FiClock, FiMapPin 
} from "react-icons/fi";
    ...
    <div className="pb-10 max-w-[1000px] mx-auto min-h-[calc(100vh-140px)] flex flex-col">
      ...
      <div className="flex justify-between items-center mb-10">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-white font-[950] tracking-tighter text-4xl leading-none">System Notifications</h1>
          </div>
          <p className="text-gray-500 font-bold text-sm">Automated telemetry alerts for critical bin levels and hardware outages.</p>
        </div>
        <Button 
      ...
      {loading ? (
        <div className="space-y-6 flex-1">
          {[...Array(3)].map((_, i) => (
            <Skeleton key={i} className="h-32 w-full rounded-3xl bg-[#11131a]" />
          ))}
        </div>
      ) : alerts.length === 0 ? (
        ...
        <div className="flex-1 flex items-center justify-center p-8">
           <div className="w-full max-w-2xl bg-[#11131a] border-2 border-dashed border-[#1e2029] rounded-[3rem] p-16 text-center flex flex-col items-center shadow-2xl relative overflow-hidden group">
              ...
              <h2 className="text-white text-4xl font-[950] tracking-tighter mb-4">All Systems Nominal</h2>
              <p className="text-gray-500 max-w-md mx-auto font-bold leading-relaxed">
                Telemetry looks good. There are no critical alerts, overflowing bins, or offline sensors detected in the network.
              </p>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                           <span className={`${style.text} text-[10px] font-black uppercase tracking-[0.2em]`}>{alert.priority} Priority</span>
                           <span className="text-gray-700 font-bold">•</span>
                           <span className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em]">{alert.type}</span>
                        </div>
                        <h3 className="text-white text-2xl font-[950] tracking-tighter mb-2">{alert.type} Detected</h3>
                        <div className="flex flex-wrap items-center gap-y-2 gap-x-5">
                           <div className="flex items-center gap-2 text-gray-500">
                              <FiMapPin size={14} className="text-gray-700" />
                              <span className="text-xs font-bold tracking-tight">{alert.bins?.location || "Unknown Location"}</span>
                           </div>
                           <div className="flex items-center gap-2 text-gray-500">
                              <FiClock size={14} className="text-gray-700" />
                              <span className="text-xs font-bold tracking-tight uppercase tracking-widest">{formatRelativeTime(new Date(alert.timestamp))}</span>
                           </div>
