...
import { 
  FiFileText, FiDownloadCloud, FiSettings, FiClock, 
  FiLock, FiChevronDown, FiRefreshCw
} from "react-icons/fi";
import { BsFileEarmarkText, BsFileEarmarkPdf, BsFileEarmarkSpreadsheet } from "react-icons/bs";
...
export default function ReportsPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (format !== "csv") {
        toast.info(`Generating as CSV instead, complex ${format.toUpperCase()} generation is pending backend implementation.`);
      }

      // Convert JSON Array to raw CSV string
      const headers = Object.keys(dataToExport[0]).join(",");
      const csvRows = dataToExport.map(row => 
        Object.values(row).map(val => `"${val}"`).join(",") // Wrap values in quotes to escape commas
      );
    ...
    <div className="pb-10 max-w-[1200px] mx-auto min-h-[calc(100vh-140px)] flex flex-col space-y-10">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-white font-[950] tracking-tighter text-4xl leading-none">System Reports</h1>
        </div>
        <p className="text-gray-500 font-bold text-sm">Generate, export, and download comprehensive network and staff data.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-[#11131a] border border-[#1e2029]/60 rounded-[2.5rem] p-10 shadow-2xl flex flex-col">
            <div className="flex items-center gap-3 mb-10">
                ...
                <h2 className="text-white text-xl font-[950] tracking-tighter">Configuration Engine</h2>
            </div>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        >
                            ...
                            <span className={`text-xs font-black tracking-tight ${format === "csv" ? "text-[#10b981]" : "text-gray-400"}`}>CSV Data</span>
                        </button>
                        <button 
                        ...
                        >
                            ...
                            <span className={`text-xs font-black tracking-tight ${format === "pdf" ? "text-white" : "text-gray-400"}`}>PDF Document</span>
                        </button>
                        <button 
                        ...
                        >
                            ...
                            <span className={`text-xs font-black tracking-tight ${format === "excel" ? "text-white" : "text-gray-400"}`}>Excel Sheet</span>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div className="lg:col-span-5 bg-[#11131a] border border-[#1e2029]/60 rounded-[2.5rem] p-10 shadow-2xl flex flex-col">
            <div className="flex items-center gap-3 mb-10 pb-6 border-b border-[#1e2029]/60">
                ...
                <h2 className="text-white text-xl font-[950] tracking-tighter">System Data Insights</h2>
            </div>

            <div className="flex-1 space-y-8">
                <div className="bg-[#0a0b0f] bg-opacity-50 rounded-2xl p-6 border border-[#1e2029]/60">
                     <p className="text-gray-400 text-sm leading-relaxed font-medium">
                         All reports are pulled directly from live telemetry systems and the raw Supabase database in real-time.
                     </p>
                </div>
                
                <div className="bg-[#0a0b0f] bg-opacity-50 rounded-2xl p-6 border border-[#1e2029]/60">
                     <p className="text-gray-400 text-sm leading-relaxed font-medium">
                         Applying accurate date constraints ensures lightweight exports and keeps your local data focused on actionable metrics.
                     </p>
                </div>
            </div>
            ...
            <div className="mt-8 bg-yellow-400/5 border border-yellow-400/10 rounded-2xl p-5 flex gap-4">
                <FiLock className="text-yellow-400 flex-shrink-0 mt-0.5" size={16} />
                <div>
                    <h4 className="text-yellow-400 text-xs font-black tracking-widest uppercase mb-1.5">Data Security Note</h4>
                    <p className="text-gray-500 text-xs font-bold leading-relaxed">
                        All exported records contain sensitive operational data. Ensure downloaded files are stored on secure, encrypted local drives.
                    </p>
                </div>
            </div>
        </div>
